...
function main() {
    ...
    $("input[name=busqueda]").keyup(function() {
        ...
        blog(textobuscar, 1, valoroption);
    });
    $("body").on("click", ".paginacion li a", function(e) {
        ...
        valoroption = $("#cantidad").val();
        blog(valorBuscar, valorhref, valoroption);
    });
    $("#cantidad").change(function() {
        valoroption = $(this).val();
        valorBuscar = $("input[name=busqueda]").val();
        blog(valorBuscar, 1, valoroption);
    });
}

function blog(valorBuscar, pagina, cantidad) {
    $.ajax({
        url: "Controller_blog/listar_blog",
        type: "POST",
        data: {
            buscar: valorBuscar,
            nropagina: pagina,
...
var i = 0
            $.each(response.blog, function(key, item) {
                ...
                filas += "<td><center>" + item.categoria + "</center></td>"
                filas += "<td><center>" + item.titulo + "</center></td>"
                filas += "<td><center><a style='background:#132D38;color:white;' class='btn btn-sm' href='Controller_blog/vista_textarea/" + item.id_articulo + "'>Ingresar-Editar Texto</a><button type='button' href=" + item.id_articulo + " data-target='.bs-example-modal-lg' class='btn btn-sm' data-deleteImg=" + item.foto + " id='delteBtnId' data-delteBtnId=" + item.id_articulo + "><i style='color:#D91F1F;' class='fa fa-trash'></i></button>";
                filas += "<button type='button' class='btn btn-sm'  id='editBtnId' data-editBtnId=" + item.id_articulo + "><i style='color:#2B77A8;' class='fa fa-pencil'></button></center>"
                filas += "</td></tr>";
            });
            if (response.cabecera == '') {
                filas += "<tr><td colspan='3'>No hay datos</td>"
            }
            $("#tbblog tbody").html(filas);
...
$(document).on("click", "#delteBtnId", function(e) {
    ...
    swal({
        ...
        if (ifWillDelete) {
            ...
            var action = 'delete';
            $.ajax({
                url: "Controller_blog/eliminar_blog",
                method: "POST",
                data: {
                    deleteImg: deleteImg,
        ...
        $(document).on("click","#editBtnId",function(e){
            ...
            $.ajax({
                url:"Controller_blog/linea_actualizar",
                method:"POST",
    ...
    if (categoria == '') {
        ...
        swal({
            title: "Selecciona un redactor",
            icon: "warning"
        });
    }else{
                $.ajax({
                    url:"Controller_blog/actualizar_crear_blog",
                    method:"POST",
