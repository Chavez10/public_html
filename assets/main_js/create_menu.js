 ...
 function menu(valorBuscar, pagina, cantidad) {
     $.ajax({
         url: "Controller_menu/listar_menu",
         type: "POST",
         data: {
             buscar: valorBuscar,
             nropagina: pagina,
         ...
         success: function(response) {
             ...
             $.each(response.menu, function(key, item) {
                i++;
                 ...
                 filas += "<td><button type='button' class='btn btn-sm' id='delteBtnId' data-delteBtnId=" + item.id_menu + "><i style='color:#D91F1F;' class='fa fa-trash'></i></button>";
                 filas += "<button type='button' class='btn btn-sm' id='editBtnId' data-editBtnId=" + item.id_menu + "><i style='color:#2B77A8;' class='fa fa-pencil'></button></td></tr>";
             });
 ...
 $(document).on("click", "#delteBtnId", function(e) {
     ...
     swal({
         ...
         if (ifWillDelete) {
             ...
             var action = 'delete';
             $.ajax({
                 url: "Controller_menu/eliminar_menu",
                 method: "POST",
                 data: {
 ...
 $(document).on("click", "#editBtnId", function(e) {
     ...
     $.ajax({
         url: "Controller_menu/linea_actualizar",
         method: "POST",
         data: {
             editBtnId: editBtnId,
             action: action
 ...
 $(document).on("submit", "#developer_cu_form", function(e) {
     ...
     if (menu == '') {
         ...
         $.ajax({
             url: "Controller_menu/actualizar_crear_menu",
             method: "POST",
             ...
             success: function(data) {
                 if (data.trim() == 'created') {
                     swal({
                         title: "Menu registrada correctamente",
                         icon: "success"
                     });
                 ...
                 if (data.trim() == 'update') {
                     swal({
                         title: "Menu actualizada correctamente",
                         icon: "success"
                     });
                     $("#developer_cu_form")[0].reset();
                 }
